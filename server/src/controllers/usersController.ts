...
class UserController {
  ...
  public async register(req: Request, res: Response): Promise<void> {
    ...
    await pool.query('INSERT INTO `users` (`id_user`, `email`, `password`, `admin`, `nombres`, `A_paterno`, `A_materno`, `FK_Fecha_Birthday`) VALUES (NULL, ?, ?, ?, ?, ?, ?, NULL)', [req.body.email,req.body.password, req.body.admin, req.body.nombres, req.body.A_paterno, req.body.A_materno],async function(err, result, fields) {
      if (err) throw err;
      await pool.query('SELECT * FROM users WHERE email = ? ', [req.body.email],async function (err, result, fields) {
        if (err) throw err;
        await pool.query('INSERT INTO `birthday` (`ID_Birthday`, `Fecha`, `FK_User`, `FK_Personal`) VALUES (NULL,?,?, NULL) ', [req.body.birthday,result[0].id_user],async function (err, result, fields) {
          if (err) throw err;
          await pool.query('UPDATE `users` SET `FK_Fecha_Birthday` = ? WHERE id_user=(SELECT FK_User FROM `birthday` where ID_Birthday =?)', [result.insertId,result.insertId],async function (err, result, fields) {
            if (err) throw err;
            await pool.query('SELECT * FROM users WHERE email = ? ', [req.body.email],async function (err, result, fields) {
              if (err) throw err;
              return res.json(result[0]);
  ...
  public async login(req: Request, res: Response): Promise<any> {
    await pool.query('SELECT * FROM users WHERE email = ? UNION SELECT * FROM users WHERE nick = ?', [req.body.email, req.body.email], function (err, result, fields) {
      if (err) throw err;
      if (result.length == 1) {
  ...
  public async isExistEmial(req: Request, res: Response): Promise<any>{
    const { email } = req.params;
      await pool.query('SELECT * FROM users WHERE email = ? UNION SELECT * FROM users WHERE nick = ?', [email, email],function(err, result, fields) {
        if (err) throw err;
        if(result.length>0){
